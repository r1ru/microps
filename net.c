...
// Outputs to the network device.
// SAFETY: `dev->ops->transmit` must be non-null.
...
// Inputs from the network device. Used by device drivers.
int net_input_handler(uint16_t type, const uint8_t *data, size_t len, struct net_device *dev) {
    ...
    for (proto = protocols; proto; proto = proto->next) {
        if (proto->type == type) {
            ...
            debugdump(data, len);
            // Notify the protocol stack that the new pakcet has arrived.
            intr_raise_irq(INTR_IRQ_SOFTIRQ);
            return 0;
        }
    }
    
    // Ignores unsupported protocols.
    return 0;
}
...
// Opens all network devices registered in `devices`.
int net_run(void) {
    struct net_device *dev;
...
// Closes all network devices registered in `devices`.
